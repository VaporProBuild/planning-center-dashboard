...
export const getButtonColor = (
  ...
  const isOutlinedProcessed = isOutlined && !['white', 'whiteDark', 'lightDark'].includes(color)

  const base = [colors.borders?.[color], colors.ring?.[color]].filter(
    (value): value is string => !!value,
  )

  if (isActive) {
    const activeColor = colors.active?.[color]
    if (activeColor) base.push(activeColor)
  } else {
    const fallback = isOutlinedProcessed ? colors.text?.[color] : colors.bg?.[color]
    if (fallback) base.push(fallback)
  }

  if (hasHover) {
    const hover = isOutlinedProcessed ? colors.outlineHover?.[color] : colors.bgHover?.[color]
